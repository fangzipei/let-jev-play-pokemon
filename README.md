...
- L2 起注入对手上下文：已确认信息与最近动作、Pikalytics 统计先验（带来源标记）、跨局经验条目；缺失时静默降级
...
| `JEV_PIKA_ENABLED` | `1` | `1` = play 启动时预拉 Pikalytics 先验并向对手注入统计假设；`0` 完全关闭 |
| `JEV_PIKA_CUTOFF` | `1760` | Pikalytics 使用率 cutoff；须为正安全整数 |
| `JEV_PIKA_DIR` | `.cache/pikalytics` | Pikalytics 先验缓存目录（按 dataDate 失效，不入库） |
...
- **L2（默认）：L1 + 条件化战术注解**。注解由当前 request 的招式、道具、特性、HP、天气/场地与 Mega 状态真实驱动（含 move 选项级机制注解与队伍级高频战术注解）；并向每只对手注入 `notes`（已确认信息、最近动作、统计先验、跨局经验，见下节），preview 附对手首发倾向引导。不是无条件粘贴固定六只的战术结论。不会新增 advisor 请求，但不保证 jev 的 token 用量或延迟不变。
...
npm run review                    # 扫描 LOG_DIR 增量入跨局经验库（规则提取）
...
src/dex/              官方 JS/JSON、来源缓存、各表独立本地补字段与空降级、Pikalytics 先验
